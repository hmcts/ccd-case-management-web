...
RUN apt-get update && apt-get install \
    ...
    --no-install-suggests \
    -y ca-certificates
COPY --chown=hmcts:hmcts package.json yarn.lock ccd-yarn-audit-known-issues yarn-audit-known-issues yarn-audit-issues yarn-audit-with-suppressions.sh .snyk bin ./
