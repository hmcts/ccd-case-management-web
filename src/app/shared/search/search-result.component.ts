...
export class SearchResultComponent implements OnChanges {
  ...
  draftPrefixOrGet(col, result): any {
    return result.case_id.startsWith(DRAFT) ? DRAFT : this.hyphenateIfCaseReferenceOrGet(col, result);
  }
  ...
  private numberOfDrafts(): number {
    return this.resultView.results.filter(_ => _.case_id.startsWith(DRAFT)).length;
  }
}
